...
void InitGpio(void)
{
   ...
   GpioCtrlRegs.GPBMUX1.bit.GPIO36=0;//设置led灯脚为GPIO 
   ...
   GpioCtrlRegs.GPAMUX1.bit.GPIO14=0;//pwm_en_2 为gpio

   GpioCtrlRegs.GPBMUX1.bit.GPIO39=0;//RDC_CS 引脚 设置为GPIO
   GpioCtrlRegs.GPAMUX1.bit.GPIO12=0;//设置 alarm为gpio

   
   GpioCtrlRegs.GPAMUX1.bit.GPIO15=0;//设置ALARM_2 脚为GPIO 
   GpioCtrlRegs.GPAMUX1.bit.GPIO13=0;//设置DC_ON 脚为GPIO 
   GpioCtrlRegs.GPAMUX2.bit.GPIO16=0;// 设置DC_ON_2 脚为GPIO 

   GpioCtrlRegs.GPAMUX2.bit.GPIO17=0;//设置CS_FM25 脚为GPIO 
   ...
   GpioCtrlRegs.GPBMUX1.bit.GPIO41=0;//设置为GPIO U
   GpioCtrlRegs.GPBMUX1.bit.GPIO42=0;//设置为GPIO V
   GpioCtrlRegs.GPBMUX1.bit.GPIO43=0;//设置为GPIO W

   GpioCtrlRegs.GPBMUX1.bit.GPIO45=0;//设置为GPIO U_2
   GpioCtrlRegs.GPBMUX1.bit.GPIO46=0;//设置为GPIO V_2
   GpioCtrlRegs.GPCMUX2.bit.GPIO81=0;//设置为GPIO W_2
   ...
   GpioCtrlRegs.GPBDIR.bit.GPIO36 = 1;      // 设置LED灯脚为输出
   ...
   GpioCtrlRegs.GPADIR.bit.GPIO14= 1 ;//PEM_EN_2为输出

   GpioCtrlRegs.GPBDIR.bit.GPIO39 = 1;      // RDC_CS 引脚 设置为输出 
   GpioCtrlRegs.GPADIR.bit.GPIO12=0;//设置alarm为输入
   GpioCtrlRegs.GPADIR.bit.GPIO15 = 0;      //设置alarm_2为输入
   DC_ON_1;
    GpioCtrlRegs.GPADIR.bit.GPIO13= 1;      // 设置DC_ON为输出
   GpioCtrlRegs.GPADIR.bit.GPIO16 = 1;      // 设置DC_ON_2为输出
DC_ON2_1;
   GpioCtrlRegs.GPADIR.bit.GPIO17  = 1;      // 设置CS_FM25为输出
   ...
   GpioCtrlRegs.GPBDIR.bit.GPIO41=0;//输入 U
   GpioCtrlRegs.GPBDIR.bit.GPIO42=0;//输入 V
   GpioCtrlRegs.GPBDIR.bit.GPIO43=0;//输入 W

   GpioCtrlRegs.GPBDIR.bit.GPIO45=0;//输入  U_2
   GpioCtrlRegs.GPBDIR.bit.GPIO46=0;//输入  V_2
   GpioCtrlRegs.GPCDIR.bit.GPIO81=0;//输入  W_2
   ...
   GpioDataRegs.GPBDAT.bit.GPIO36=1;//设置LED灯引脚输出高电平
   ...
   GpioDataRegs.GPBDAT.bit.GPIO39=1;// 
    ...
    GpioDataRegs.GPADAT.bit.GPIO13=1;//设置DC_ON 输出高电平,为低电平时 母线继电器闭合
     GpioDataRegs.GPADAT.bit.GPIO16=1;//设置DC_ON_2 输出高电平,为低电平时 母线继电器闭合
...
void  IPM_BaoHu(void)
{
    ...
    j=GpioDataRegs.GPADAT.bit.GPIO12;//读取故障位
    if(j==0)
    {
        i++;
        if(i==2)
    ...
    {i=0;
       ...
       jj=GpioDataRegs.GPADAT.bit.GPIO15;//读取故障位
    if(jj==0)
    {
        ii++;
        if(ii==2)
